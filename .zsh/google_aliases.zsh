...
hg() {
  ...
  if [[ "$1" == "amend" || "$1" == "commit" ]]; then
    paths=($(hg st | cut -d' ' -f2 | rev | cut -d'/' -f2- | rev | sort -u))
    if [[ ! "${#paths[@]}" -eq 0 ]]; then
      printf "[+] Running glaze\n"
      build_paths=( "${paths[@]/#///}"  )  # prepend // to each path
      glaze -p "${build_paths[@]}"
    fi
...
# Run a SQL query against a Capacitor file.
cquery() {
  if [[ "$#" -ne 2 ]]; then
...
PROD_PLACER_BASEPATH="/placer/prod/home/tpsi-placer"
TEST_PLACER_BASEPATH="/placer/test/home/jsomerville"

# Interactively select a file from Placer.
get_placer_path() {
  if [[ "$1" == "prod" ]]; then
    base_path=$PROD_PLACER_BASEPATH
  elif [[ "$1" == "test" ]]; then
    base_path=$TEST_PLACER_BASEPATH
  else
    printf "Usage:\n  get_placer_path [prod|test]\n" >&2
    return 1
  fi
...
# Delete multiple file sets from Placer.
deletefilesets() {
  if [[ ! "$1" =~ ^("prod"|"test")$ ]]; then
    printf "Usage:\n deletefilesets (test|prod)"
    return 1
  fi
  base_path=$PROD_PLACER_BASEPATH
  if [[ "$1" == "test" ]]; then; base_path=$TEST_PLACER_BASEPATH; fi

  printf "[+] Select which file set(s) to delete (select multiple with TAB):\n"
  filesets=($(placer list_filesets "${base_path}" | fzf --ansi -m))
  placer delete -f "${filesets[@]}"
}
