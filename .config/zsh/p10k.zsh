...
() {
  ...
  #################### [ hgstatus: custom fig repo status ] ####################
  function prompt_hgstatus() {
    [[ -n $FIGSTATUS_CITC ]] || return
    p10k segment -f '#abafab' -e -i '' -t '%B%F{#abafab}$FIGSTATUS_CITC %F{3}$FIGSTATUS_MODIFIED%f%b'
    # p10k segment -f '#abafab' -e -i '' -t '%B%F{#abafab}$(get_fig_prompt)%f%b'
  }
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      res+="%7F${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${branch//\%/%%}"
    fi
    ...
    (( VCS_STATUS_NUM_CONFLICTED ))     && res_stat+="~"
    (( VCS_STATUS_NUM_STAGED     ))     && res_stat+="+"
    (( VCS_STATUS_NUM_UNSTAGED   ))     && res_stat+="!"
    (( VCS_STATUS_NUM_UNTRACKED  ))     && res_stat+="?"
    (( VCS_STATUS_HAS_UNSTAGED == -1 )) && res_stat+="─"
